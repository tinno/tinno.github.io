...
var mode = "single";
var default_path = "F:\\dev_branches";
...
$(".btn-quick").click(function(){
	...
	var path = $("#input_link").val().split(rootPath)[1].replace(/\\/g,"\/");
	var link_ui = 'http://ui.tenpay.com' + path;
...
$(".btn-many").click(function(){
	...
	var links_ui = "<legend>本地ui static</legend>",
		links_tid = "<legend>TID地址</legend>",
		links_ptmp = "<legend>PTMP地址</legend>";
	var temp_link = '<p><a href="{link}" target="_blank">{link}</a></p>';
	for (var i = 0; i < array.length; i++) {	
		var path = array[i].split(rootPath)[1].replace(/\\/g,"\/");
		links_ui += temp_link.replace(/{link}/g,'http://ui.tenpay.com' + path);
